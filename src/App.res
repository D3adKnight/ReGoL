...
let make = (~gridSize=gridSize) => {
  ...
  let toggleCell = (x, y) => setGrid(grid => Game.toggleCell(~grid, ~size=gridSize, ~x, ~y))
  ...
  let style = ReactDOM.Style.make(~display="flex", ~flexDirection="row", ~margin="0 -10px", ())

  <div>
    <h2> {React.string("Game of Life")} </h2>
    <div style>
      <div style={ReactDOM.Style.make(~margin="0 10px", ())}>
        <Controls onClear onStart onRandom startLabel /> <Grid grid gridSize toggleCell />
      </div>
      <Info />
    </div>
  </div>
